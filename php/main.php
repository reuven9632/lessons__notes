<?php
  ...
  if (isset($_POST["send"])) {            //если нажата кнопка отправить то начинаем выполнение кода
    ...
    // ! создаю переменные сессии для того чтобы если пользователь неверно ввел часть данных при перезагрузке страницы ему не притшлось заново их вводить, так как переменные сессии буду фозвращать в код html формы в атрибут value
    $_SESSION["from"] = $from;
    $_SESSION["to"] = $to;
    $_SESSION["subject"] = $subject;
    $_SESSION["message"] = $message;

    //todo создаю  пустые переменные для вывода текста ошибки
    $error_from = "";
    $error_to = "";
    $error_subject = "";
    $error_message = "";
